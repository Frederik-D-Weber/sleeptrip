function [cfg, artifact] = ft_artifact_threshold_peak(cfg, data)

% FT_ARTIFACT_THRESHOLD_PEAK reads the interesting segments of data from file and identifies
% artifacts by means of thresholding of the preprocessed raw data, if wanted the values could be z-transformed before thresholding.
% Depending on the preprocessing options, this method will be sensitive to EOG, muscle or
% jump artifacts and detection of events.  This procedure only works on continuously recorded data.
%
% Use as
%   [cfg, artifact] = ft_artifact_threshold_peak(cfg)
...
%   [cfg, artifact] = ft_artifact_threshold_peak(cfg, data)
...
% and
%   cfg.artfctdef.threshold.channel     (default = {})
%   cfg.artfctdef.threshold.cutoff      (default = 0)
%   cfg.artfctdef.threshold.trlpadding  (default = 0)
%   cfg.artfctdef.threshold.fltpadding  (default = 0)
%   cfg.artfctdef.threshold.artpadding  (default = 0)
%   cfg.artfctdef.threshold.applyztransfrom (default = 'no')
%
...
%   cfg.artfctdef.threshold.artfctpeak  = 'yes' or 'no'
%   cfg.artfctdef.threshold.interactive = 'yes' or 'no'
%
% If you specify cfg.artfctdef.threshold.artfctpeak='yes', the maximum value of the
% artifact within its range will be found and saved into cfg.artfctdef.threshold.peaks.
%
% If you specify cfg.artfctdef.threshold.interactive='yes', a GUI will be started and
...
%
%     downarrow         : Shift the threshold down
%     z                 : Specify the threshold
%     uparrow           : Shift the threshold down
%
% Use also, e.g. as input to DSS option of ft_componentanalysis
% cfg.artfctdef.threshold.artfctpeakrange=[-0.25 0.25], for example to indicate range
% around peak to include, saved into cfg.artfctdef.threshold.dssartifact. The default is
...
%   cfg.artfctdef.threshold.lpfilter      = 'no' or 'yes'  lowpass filter
%   cfg.artfctdef.threshold.hpfilter      = 'no' or 'yes'  highpass filter
%   cfg.artfctdef.threshold.bpfilter      = 'no' or 'yes'  bandpass filter
%   cfg.artfctdef.threshold.bsfilter      = 'no' or 'yes'  bandstop filter for line noise removal
%   cfg.artfctdef.threshold.dftfilter     = 'no' or 'yes'  line noise removal using discrete fourier transform
%   cfg.artfctdef.threshold.medianfilter  = 'no' or 'yes'  jump preserving median filter
%   cfg.artfctdef.threshold.lpfreq        = lowpass  frequency in Hz
%   cfg.artfctdef.threshold.hpfreq        = highpass frequency in Hz
%   cfg.artfctdef.threshold.bpfreq        = bandpass frequency range, specified as [low high] in Hz
%   cfg.artfctdef.threshold.bsfreq        = bandstop frequency range, specified as [low high] in Hz
%   cfg.artfctdef.threshold.lpfiltord     = lowpass  filter order
%   cfg.artfctdef.threshold.hpfiltord     = highpass filter order
%   cfg.artfctdef.threshold.bpfiltord     = bandpass filter order
%   cfg.artfctdef.threshold.bsfiltord     = bandstop filter order
%   cfg.artfctdef.threshold.medianfiltord = length of median filter
%   cfg.artfctdef.threshold.lpfilttype    = digital filter type, 'but' (default) or 'firws' or 'fir' or 'firls'
%   cfg.artfctdef.threshold.hpfilttype    = digital filter type, 'but' (default) or 'firws' or 'fir' or 'firls'
%   cfg.artfctdef.threshold.bpfilttype    = digital filter type, 'but' (default) or 'firws' or 'fir' or 'firls'
%   cfg.artfctdef.threshold.bsfilttype    = digital filter type, 'but' (default) or 'firws' or 'fir' or 'firls'
%   cfg.artfctdef.threshold.detrend       = 'no' or 'yes'
%   cfg.artfctdef.threshold.demean        = 'no' or 'yes'
%   cfg.artfctdef.threshold.baselinewindow = [begin end] in seconds, the default is the complete trial
%   cfg.artfctdef.threshold.hilbert       = 'no' or 'yes'
%   cfg.artfctdef.threshold.rectify       = 'no' or 'yes'
%
...
cfg.artfctdef.threshold.method      = ft_getopt(cfg.artfctdef.threshold, 'method',       'all');
cfg.artfctdef.threshold.ntrial      = ft_getopt(cfg.artfctdef.threshold, 'ntrial',       10);
cfg.artfctdef.threshold.channel     = ft_getopt(cfg.artfctdef.threshold, 'channel',      {});
cfg.artfctdef.threshold.trlpadding  = ft_getopt(cfg.artfctdef.threshold, 'trlpadding',   0);
cfg.artfctdef.threshold.fltpadding  = ft_getopt(cfg.artfctdef.threshold, 'fltpadding',   0);
cfg.artfctdef.threshold.artpadding  = ft_getopt(cfg.artfctdef.threshold, 'artpadding',   0);
cfg.artfctdef.threshold.applyztransfrom  = ft_getopt(cfg.artfctdef.threshold, 'applyztransfrom', 'no');
cfg.artfctdef.threshold.interactive = ft_getopt(cfg.artfctdef.threshold, 'interactive',  'no');
cfg.artfctdef.threshold.cumulative  = ft_getopt(cfg.artfctdef.threshold, 'cumulative',   'yes');
cfg.artfctdef.threshold.artfctpeak  = ft_getopt(cfg.artfctdef.threshold, 'artfctpeak',   'no');
cfg.artfctdef.threshold.artfctpeakrange  = ft_getopt(cfg.artfctdef.threshold, 'artfctpeakrange',[0 0]);
...
cfg.artfctdef.threshold = ft_checkconfig(cfg.artfctdef.threshold, 'renamed', {'sgn',      'channel'});
cfg.artfctdef.threshold = ft_checkconfig(cfg.artfctdef.threshold, 'renamed', {'feedback', 'interactive'});

if isfield(cfg.artfctdef.threshold, 'artifact')
  fprintf('threshold artifact detection has already been done, retaining artifacts\n');
  artifact = cfg.artfctdef.threshold.artifact;
  return
end
...
% flag whether to compute z-value/threshold per trial or not, rationale being that if
...
pertrial    = strcmp(cfg.artfctdef.threshold.method, 'trial');
demeantrial = strcmp(cfg.artfctdef.threshold.method, 'trialdemean');
if pertrial
  if isfield(cfg.artfctdef.threshold, 'ntrial') && cfg.artfctdef.threshold.ntrial>0
    pertrial = cfg.artfctdef.threshold.ntrial;
  else
    ft_error('you should specify cfg.artfctdef.threshold.ntrial, and it should be > 0');
  end
end
...
if ~hasdata
  ...
  if cfg.artfctdef.threshold.trlpadding > 0
...
trlpadding    = round(cfg.artfctdef.threshold.trlpadding*hdr.Fs);
fltpadding    = round(cfg.artfctdef.threshold.fltpadding*hdr.Fs);
artpadding    = round(cfg.artfctdef.threshold.artpadding*hdr.Fs);
...
numtrl        = size(trl,1);
cfg.artfctdef.threshold.trl = trl;              % remember where we are going to look for artifacts
cfg.artfctdef.threshold.channel = ft_channelselection(cfg.artfctdef.threshold.channel, hdr.label);
sgnind        = match_str(hdr.label, cfg.artfctdef.threshold.channel);
numsgn        = length(sgnind);
thresholdsum  = strcmp(cfg.artfctdef.threshold.cumulative, 'yes');
...
for trlop = 1:numtrl
  ...
  if strcmp(cfg.memory, 'low') % store nothing in memory
    ...
    dat = preproc(dat, cfg.artfctdef.threshold.channel, offset2time(0, hdr.Fs, size(dat,2)), cfg.artfctdef.threshold, fltpadding, fltpadding);
    if istrue(cfg.artfctdef.threshold.applyztransfrom)
      ...
      dat{trlop} = preproc(dat{trlop}, cfg.artfctdef.threshold.channel, offset2time(0, hdr.Fs, size(dat{trlop},2)), cfg.artfctdef.threshold, fltpadding, fltpadding);
      if istrue(cfg.artfctdef.threshold.applyztransfrom)
          if trlop==1 && ~pertrial
...
for trlop = 1:numtrl
  if strcmp(cfg.memory, 'low') % store nothing in memory (note that we need to preproc AGAIN... *yawn*
    ...
    if istrue(cfg.artfctdef.threshold.applyztransfrom)
        ...
        zdata         = (dat - datavg(:,indvec(trlop)*ones(1,nsmp)))./datstd(:,indvec(trlop)*ones(1,nsmp));
    else
        
        zdata         = dat;  % convert the filtered data to z-values
    end
    ...
    zdata         = dat{trlop};  % convert the filtered data to z-values
    zsum{trlop}   = nansum(zdata,1);                   % accumulate the z-values over channels
    [zmax{trlop},ind] = max(zdata,[],1);            % find the maximum z-value and remember it
    zindx{trlop}      = sgnind(ind);                % also remember the channel number that has the largest z-value
  end
...
for sgnlop=1:numsgn
 ...
 sumsqr = 0;
 numsmp = 0;
 fprintf('searching channel %s ', cfg.artfctdef.threshold.channel{sgnlop});
 for trlop = 1:numtrl
   fprintf('.');
   if hasdata
   ...
   dat{trlop} = preproc(dat{trlop}, cfg.artfctdef.threshold.channel(sgnlop), hdr.Fs, cfg.artfctdef.threshold, [], fltpadding, fltpadding);
   % accumulate the sum and the sum-of-squares
...
opt.artcfg       = cfg.artfctdef.threshold;
opt.artval       = {};
...
opt.quit         = 0;
opt.threshold    = cfg.artfctdef.threshold.cutoff;
...
if strcmp(cfg.artfctdef.threshold.interactive, 'yes')
  set(h, 'visible', 'on');
...
if strcmp(cfg.artfctdef.threshold.artfctpeak, 'yes')
  cnt=1;
  shift=opt.trl(1,1)-1;
  for tt=1:opt.numtrl
    if tt==1
    ...
    for rr=1:size(artbegend,1)
      ...
      dssartifact(cnt,1)=max(peaks(cnt)+cfg.artfctdef.threshold.artfctpeakrange(1)*hdr.Fs,opt.trl(tt,1));
      dssartifact(cnt,2)=min(peaks(cnt)+cfg.artfctdef.threshold.artfctpeakrange(2)*hdr.Fs,opt.trl(tt,2));
  ...
  cfg.artfctdef.threshold.peaks=peaks';
  cfg.artfctdef.threshold.dssartifact=dssartifact;
end
...
cfg.artfctdef.threshold.cutoff   = opt.threshold;
