...
%FW: cfg.contour_par = parameter for contour thresholding, = 'stat' | 'prob'
%FW: cfg.contour_ths = thresholds for countour inclusion, depending on cfg.stat_par
%FW: cfg.contour_th_signs = direction of threshold, = '+' other wise negative is assumed
%FW: cfg.contour_colors = direction of threshold, = '+' other wise negative is assumed
%FW: cfg.x2range = [minX maxX] for second y axis default [min(xdata2) max(xdata2)]
%FW: cfg.y2range = [minY maxY] for second y axis default [min(ydata2) max(ydata2)]
%FW: cfg.y2label = label for second y axis default 'scale 2'
%FW: cfg.y2colors = colors for second y axis default 'b' in each line a color
...
% fw
nLines = size(ydata2,1); % each rows is a line
if ~isfield(cfg, 'contour_linewidth')
  cfg.contour_linewidth = 1.8;
end
if ~isfield(cfg, 'x2range')
  cfg.x2range = [min(xdata2) max(xdata2)];
end
if ~isfield(cfg, 'y2range')
  cfg.y2range = [min(ydata2) max(ydata2)];
end
if ~isfield(cfg, 'y2label')
  cfg.y2label = 'scale 2';
end
if ~isfield(cfg, 'y2colors')
  cfg.y2colors = repmat('b',nLines,1);
else
    if size(cfg.y2colors,1) ~= nLines
        error ('cfg.y2colors does not match number of lines/rows in ydata2');
    end
end
if ~isfield(cfg, 'y2linestyles')
    cfg.y2linestyles = repmat('-',nLines,1);
else
    if length(cfg.y2linestyles) ~= nLines
        error ('cfg.y2linestyles does not match number of lines/rows in ydata2');
    end
end
if ~isfield(cfg, 'y2linewidths')
    cfg.y2linewidths = repmat(1,nLines,1);
else
    if length(cfg.y2linewidths) ~= nLines
        error ('cfg.y2linewidths does not match number of lines/rows in ydata2');
    end
end
if ~isfield(cfg, 'y2alphas')
    cfg.y2alphas = repmat(1,nLines,1);
else
    if length(cfg.y2alphas) ~= nLines
        error ('cfg.y2alphas does not match number of lines/rows in ydata2');
    end
end
...
if ~strcmp(cfg.baseline, 'no')
  ...
  %fw
  if isfield(cfg,'contour_par')
    tempcontourparameter = data.(cfg.contour_par);
  end
  data = ft_freqbaseline(cfg, data);
  if isfield(cfg,'contour_par')
    data.(cfg.contour_par) = tempcontourparameter;
  end
  % put mask-parameter back if it is set
  if ~isempty(cfg.maskparameter)
    data.(cfg.maskparameter) = tempmask;
...
xidc = find(data.(xparam) >= data.(xparam)(xmin) & data.(xparam) <= data.(xparam)(xmax));
% % Align physical x-axis range to the array bins:
% xmin = data.(xparam)(xidc(1));
% xmax = data.(xparam)(xidc(end));
...
yidc = find(data.(yparam) >= data.(yparam)(ymin) & data.(yparam) <= data.(yparam)(ymax));
% % Align physical y-axis range to the array bins:
% ymin = data.(yparam)(yidc(1));
% ymax = data.(yparam)(yidc(end));
...
axis tight;
  ...
  xas = data.(xparam)(xidc);
  yas = data.(yparam)(yidc);% + (ymax - ymin)/(size(data.freq,2));
  % Get stat
  if (isfield(cfg,'contour')) && (strcmp(cfg.contour,'yes'))% prepare the statitics data
      Scdata = squeeze(data.(cfg.contour_par)(sellab,yidc,xidc));      
      if(strcmp(cfg.contour_type,'statistic'))
          for t = 1:(length(cfg.contour_ths))
              th = cfg.contour_ths(t);
              ccdata=zeros(size(Scdata));
              if (strcmp(cfg.contour_par,'stat'))
                  ccdata(find(abs(Scdata)>th))=1;
              elseif (strcmp(cfg.contour_par,'prob'))
                  ccdata(find(abs(Scdata)<th))=1;
              end
              
              [xas_uneven, yas_uneven, ccdata_uneven] =  uneven_correction(xas,yas,ccdata);        
              contour(xas_uneven,yas_uneven,ccdata_uneven,1,'linewidth',cfg.contour_linewidths(t),'Color', cfg.contour_colors(t,:));
          end
      elseif (strcmp(cfg.contour_type,'threshold'))
          for t = 1:(length(cfg.contour_ths))
              th = cfg.contour_ths(t);
              ccdata=zeros(size(cdata));
              if isfield(cfg, 'countour_th_signs')
                  if strcmp(cfg.sign,'+')
                      ccdata(find(cdata>th))=1;
                  else%negative asumed
                      ccdata(find(cdata.*-1>th))=1;
                  end
              else
                ccdata(find(abs(cdata)>=cfg.stat_th))=1;
              end
              [xas_uneven, yas_uneven, ccdata_uneven] =  uneven_correction(xas,yas,ccdata);        
              contour(xas_uneven,yas_uneven,ccdata_uneven,1,'linewidth',cfg.contour_linewidths(t),'Color', cfg.contour_colors(t,:));
          end
...
p1gca = gca;
p1gccb=colorbar;
y1range = [min(yas) max(yas)];
ydata2_norm = fw_normalize(ydata2, min(cfg.y2range), max(cfg.y2range), min(y1range), max(y1range));

x1range = [min(xas) max(xas)];
xdata2_norm = fw_normalize(xdata2, min(cfg.x2range), max(cfg.x2range), min(x1range), max(x1range));

for iLines = 1:nLines
    temp_color      = cfg.y2colors(iLines,:);
    temp_linestyle  = cfg.y2linestyles(iLines);
    temp_linewidth  = cfg.y2linewidths(iLines);
    temp_alpha  = cfg.y2alphas(iLines);
    temp_tag        = 'cip2';
    ft_fw_plot_line(xdata2_norm, ydata2_norm(iLines,:),'tag',temp_tag,'color',temp_color,'linestyle', temp_linestyle,'linewidth',temp_linewidth,'alpha',temp_alpha);
end;
...
hNewAxes = axes('Position',axesPosition,...  %# Place a new axes on top...
                ...
                'YLim',[min(cfg.y2range) max(cfg.y2range)],...            %#   ... and a different scale
                'YAxisLocation','right',...  %#   ... located on the right
                'XTick',[],...               %#   ... with no x tick marks
                'Box','off');                %#   ... and no surrounding box
ylabel(hNewAxes,cfg.y2label);  %# Add a label to the right y axis
p2gca = gca;

temp_p1gca_pos = get(p1gca,'Position');
temp_p2gca_pos = get(p2gca,'Position');
temp_p1gccb_pos = get(p1gccb,'Position');
temp_p1gccb_pos(1) = 0.85; % change position to the right
set(p1gccb,'Position',temp_p1gccb_pos)
temp_p1gca_pos(3) = 0.6;
temp_p2gca_pos(3) = 0.6;
set(p1gca,'Position',temp_p1gca_pos);
set(p2gca,'Position',temp_p2gca_pos);
...
function [xe, ye, ce] = uneven_correction(x,y,c)
