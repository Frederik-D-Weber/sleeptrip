function [data] = st_preprocessing(cfg, data, varargin)

% ST_PREPROCESSING reads MEG and/or EEG data according to user-specified trials
% and applies several user-specified preprocessing steps to the signals.
% TEMPORARY ADAPTION FOR SLEEPTRIP
%
% Use as
%   [data] = st_preprocessing(cfg)
% or
%   [data] = st_preprocessing(cfg, data)
%
...
%
% If you are calling ST_PREPROCESSING with the second input argument "data", then
% that should contain data that was already read from file in a previous call to
% ST_PREPROCESSING. In that case only the configuration options below apply.
%
...
% ST_PREPROCESSING with a single cfg input argument are
...
%
% See also FT_DEFINETRIAL, FT_REDEFINETRIAL, FT_APPENDDATA, FT_APPENDSPIKE, FT_PREPROCESSING
...
if hasdata
  ...
  for i=1:ntrl
    ...
    [dataout.trial{i}, dataout.label, dataout.time{i}, cfg] = preproc_fw(data.trial{i}, data.label,  data.time{i}, cfg, begpadding, endpadding);
...
%               ind_org(iFsType) = find(a(iFsType) == hdr.orig.SampleRate(rawindx_candidate),1,'first');
%               
%           end
%           [dum, indx] = sort(ind_org);
%           chancount = chancount(indx);
%           a = a(indx)
...
%           hdr.nSamples = round(hdr.nSamples*(hdr.Fs/temphdr.Fs));
%           %hdr.nTrials = round(hdr.nTrials*(hdr.Fs/temphdr.Fs));
...
%       %ST END
  ...
  for j=1:length(chnloop)
    ...
    for i=1:ntrl
      ...
      [cutdat{i}, label, time{i}, cfg] = preproc_fw(dat, hdr.label(rawindx), tim, cfg, begpadding, endpadding);
