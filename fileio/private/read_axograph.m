function [dat, hdr] = read_axograph(filname, read_data, chanindx)
...
dat = [];
if read_data
    dat = double(zeros(numel(chanindx),hdr.nPoints));
end
...
for iYCol = 1:(hdr.NDatCol-1)
    ...
    read_channel = read_data;
    
    if ~any(iYCol == chanindx);
        read_channel = false;
    else
       iDat = iDat + 1; 
    end
    ...
    if read_channel
        switch hdr.YCol(iYCol).YColType
            case 4  % column type is short
                dat(iDat,:) = double(fread(fid,hdr.YCol(iYCol).nPoints, 'int16'));
            case 5  % column type is long
                dat(iDat,:) = double(fread(fid,hdr.YCol(iYCol).nPoints, 'int32'));
            case 6  % column type is float
                dat(iDat,:) = double(fread(fid,hdr.YCol(iYCol).nPoints, 'float32'));
            case 7  % column type is double
                dat(iDat,:) = fread(fid,hdr.YCol(iYCol).nPoints, 'double');
            case 9  % 'series'
...
for iCh = 1:numel(hdr.YCol)
    hdr.labels{iCh} = hdr.YCol(iCh).YTitle;
end
hdr.labels = hdr.labels';

%reorder the channels according to the chanindx, the channel labels is
%reorederd in the higher oder functions
[dummy iReorder] = sort(chanindx);
if ~isempty(dat)
    dat = dat(iReorder,:);
end
