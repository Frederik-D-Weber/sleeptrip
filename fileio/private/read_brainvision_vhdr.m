...
% Copyright (C) 2019 Frederik D. Weber modified for SleepTrip
%
...
if strcmpi(vhdr.DataFormat, 'binary')
 ...
 switch lower(vhdr.BinaryFormat)
    ...
    case 'ieee_float_32';
      NBytes = 4;
    otherwise 
     ft_error('unkown Binary format %s of brainvision file', lower(vhdr.BinaryFormat));
  end
  ...
  vhdr.nSamples = fix(info.bytes./(vhdr.NumberOfChannels*NBytes));
