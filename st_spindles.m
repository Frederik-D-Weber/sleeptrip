...
%                         If this value is NaN then the tables of the
%                         res_***.table have no rows
%
...
if isnan(cfg.centerfrequency)
    res_filter.table = cell2table(cell(0,numel(tempvarnames)), 'VariableNames', tempvarnames);
else
...
tempvarnames = {...
    'channel',...
    'count','density_per_epoch','density_per_minute','mean_duration_seconds','mean_amplitude_trough2peak_potential',...
...
if isnan(cfg.centerfrequency) || (nChannels < 1)
    ...
    threshold_stages_col = threshold_stages_col(:);
...
if isnan(cfg.centerfrequency) || (nChannels < 1)
    ...
    if isempty(output)
        ...
        res_event.table = table(...
            ...
            [output{:,25}]',[output{:,26}]',repmat(lengthsAcrossROIsSeconds,size(output,1),1),...
            'VariableNames',tempvarnames);
    end
end
data = [];%clear
