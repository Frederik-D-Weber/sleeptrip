...
function [remDens, exactlyMarkedREMs, meanREMSpeed, meanREMWidth, EOG1, EOG2, EOG1_lp, EOG2_lp, EOG1_lphp, EOG2_lphp, EOG1_lp_dev, EOG2_lp_dev, EOG1_lphp_dev, EOG2_lphp_dev, eogNoise, thrBS, thrRS] ...
    = paperSciRemDetectorAdjustableThreshold(EOG1, EOG2, orgSampling, epLen, eogClass, countAll,epochOfInterestToPlot, RelaxedThresholdFactorOfBasicThreshold, BasicThresholdFactorOfEOGnoise, amplitudemax)
...
for i = 1:gap:sigLen - gap % -> for each epoch
    ...
    for jj = thrRange*remSearchGap:1:length(c1) - thrRange*remSearchGap
        ...
        %amplitude has to be low enough (below +/-amplitudemax uV)
        ...
        if  (abs(rem1(jj)) == thrBS  || ...
                ...
                ( abs(sigPiece1(jj)) < amplitudemax || plusminusREM1(jj) == rem1(jj)) )) && abs(c2(jj) - rem1(jj)) > thrRS
        ...
        if (abs(rem2(jj)) == thrBS || ...
                ...
                ( abs(sigPiece2(jj)) < amplitudemax || plusminusREM2(jj) == rem2(jj)))) && abs(c1(jj) - rem2(jj)) > thrRS
        ...
        if  (abs(rem1(jj)) == thrBS  || ... % 1
                ...
                ( abs(sigPiece1(jj)) < amplitudemax || plusminusREM1(jj) == rem1(jj)) )) && ... % 2 signal on the same side as direction of movement or amplitude < amplitudemax uV
            ...
            while remStart < remEnd
                ...
                if ( abs(rem2(remStart)) == thrBS || ...  % 1
                        ...
                        ( abs(sigPiece2(remStart)) < amplitudemax || plusminusREM2(remStart) == rem2(remStart))) ) && ...   % 2
