function [filt] = ft_fw_preproc_bandpassfilter(dat,Fs,Fbp,N,type,dir,instabilityfix,cfg)
...
%           if rem(N,2)==1,   N=N+1;    end
%       end
      transitionsWidthHz_low = cfg.StopToPassTransitionWidth_bp;
      transitionsWidthHz_high = cfg.PassToStopTransitionWidth_bp;


      Fbp_low_nq = min(Fbp)/Fn;
      Fbp_high_nq = max(Fbp)/Fn;

      Fbp_low_trans_nq =(min(Fbp)-transitionsWidthHz_low)/Fn;
      Fbp_high_trans_nq =(max(Fbp)+transitionsWidthHz_high)/Fn;

      f = [0 Fbp_low_trans_nq Fbp_low_nq Fbp_high_nq Fbp_high_trans_nq 1];
      z = [0 0 1 1 0 0];
      A = 1;
      B = firls(N,f,z); % requires Matlab signal processing toolbox
  case 'brickwall'
